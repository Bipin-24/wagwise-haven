...
type NavLink = { label: string } & (
  | { to: "/" | "/services" | "/about" }
  | { to: "/services/$slug"; slug: string }
);

const links: NavLink[] = [
  { to: "/", label: "Home" },
  { to: "/services", label: "Services" },
  { to: "/services/$slug", slug: "boarding", label: "Boarding" },
  { to: "/services/$slug", slug: "training", label: "Training" },
  { to: "/services/$slug", slug: "vet", label: "Vet Care" },
  { to: "/services/$slug", slug: "food", label: "Food" },
  { to: "/about", label: "About Us" },
];

function NavItem({ link, className, onClick }: { link: NavLink; className: string; onClick?: () => void }) {
  if (link.to === "/services/$slug") {
    return (
      <Link
        to="/services/$slug"
